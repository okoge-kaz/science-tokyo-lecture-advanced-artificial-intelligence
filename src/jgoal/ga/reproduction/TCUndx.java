...
public class TCUndx<X extends ICRealSolution> implements Serializable {
   ...
   * @since 2 hmkz
   */
  public TCUndx(@ACParam(key = "Random") ICRandom random) {
  ...
  public void makeOffspring(TCSolutionSet<X> parents, int noOfKids, TCSolutionSet<X> kids) {
    ...
    assert dimension == p2.getRowDimension()
            && dimension == p3.getRowDimension()
            && p2.getRowDimension() == p3.getRowDimension()
        : "The dimensions of parents should be the same.";
  ...
  /**
   * Calculates the normalized vector of the primary search component which is a line from the
   * parent 1 to 2 and the standard deviation of them.
   *
   * @param v1 the first parent
   * @param v2 the second parent
   * @since 2 hmkz
  ...
  private void calcUnitVectorAndStandardDeviationForPrimaryComponent(TCMatrix v1, TCMatrix v2) {
    ...
    if (isEquals(d1, 0.0)) isParent1EqualToParent2 = true;
    else isParent1EqualToParent2 = false;
    fEVector.times(1.0 / d1); // the normalized vector of the primary search component
  }

  /**
   * Calculates the normalized vector of the secondary search component which is a line from the
   * primary component to the third parent and the standard deviation of them.
   *
   * @param v1 the first parent
  ...
  private void calcUnitVectorAndStandardDeviationForSecondaryComponent(TCMatrix v1, TCMatrix v3) {
    ...
    TCMatrix perpendicular =
        new TCMatrix(v1v3); // the perpendicular vector from v3 to the primary component
    ...
    fSigma2 =
        fBeta
            * d2
            / Math.sqrt(
                (double) v1.getRowDimension()); // the normalized vector of the secondary component
  }

  /**
   * 誤差を考慮して比較 20110707 jgoal2010のTCComparatorに実装されているが，移植するか不明のためここに作成．
   *
   * @author uemura
   * @param d1
   * @param d2
