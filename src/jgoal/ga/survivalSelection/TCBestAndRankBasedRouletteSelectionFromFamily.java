...
/**
 * A survival selection that chooses two individuals from the family (an union of the parents and
 * offspring) and then replaces the first two of the parents with the chosen individuals. The choice
 * of the two is done by different manners for each. The first individual is chosen by elitism and
 * the second is done by the rank-based roulette wheel selection from the rest.
 *
 * @param <X> the type of individuals involved in this selection
 * @since 2
 * @author hmkz, isao
 */
public class TCBestAndRankBasedRouletteSelectionFromFamily<X extends ICSolution>
