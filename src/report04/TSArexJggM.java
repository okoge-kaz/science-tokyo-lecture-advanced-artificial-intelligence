...
/**
 * This program executes AREX/JGG three trials, changing a random seed.
 * It saves a transition of the best evaluation value at each generation in a log file.
...
public class TSArexJggM {
	...
	private static void executeOneTrial(TSArexJgg ga, long maxEvals, TCTable log, String trialName, int trialNo) {
		...
		putLogData(log, trialName, trialNo, logIndex, noOfEvals, best); //Save the information of the initial population in the log table.
		...
		while (best > 1e-7 && noOfEvals < maxEvals) { //the termination condition : the best evaluation value is smaller than 10^-7 or the number of evaluation is larger than the maximum number of evaluation.
			...
			if (loopCount % 10 == 0) { //Saves the best evaluation value in the log table at every 10 steps.
				putLogData(log, trialName, trialNo, logIndex, noOfEvals, best);			
				++logIndex; //Increments the line number of the log table.
			}
			++loopCount; //Increments the loop counter.
		}
		putLogData(log, trialName, trialNo, logIndex, noOfEvals, best); //Saves the information of the last population in the log table.	
		System.out.println("TrialNo:" + trialNo + ", NoOfEvals:" + noOfEvals + ", Best:" + best); //Displays the trial number, the number of evaluation and the best evaluation value.
	}
	...
	public static void main(String[] args) throws IOException {
		...
		double min = +1.00; //The minimum value of the initialization area.
		...
		String trialName = "ArexJggOffsetKTabletP14K5"; //The trial name, which is used for the log filename.
		...
		for (int trial = 0; trial < maxTrials; ++trial) {
			...
			executeOneTrial(ga, maxEvals, log, trialName, trial); //Executes one trial.
		}
		log.writeTo(logFilename); //Outputs the log table containing the results of the three trials to the log file.
	}
