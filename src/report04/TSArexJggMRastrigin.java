...
/**
 * This program executes AREX/JGG three trials, changing a random seed. It saves a transition of the
 ...
 * Dimension: n=20. - Initial region: [+1,+5]^n， - Population size: 14n - The number of offspring:
 * 5n - The maximum number of evaluation: 4n x 1e4 - The evaluation value for termination: 1.0 x
 * 1e-7 - Log filename: ArexJggOffsetRastriginP14K5.csv
 *
 * @author isao
 */
public class TSArexJggMRastrigin {
  ...
  private static void evaluate(TCSolutionSet<TSRealSolution> population) {
    for (TSRealSolution s : population) {
      double eval = rastrigin(s.getVector()); // Obtains an evaluation value of k-tablet function.
   ...
   */
  private static double rastrigin(TCMatrix x) {
    int n = x.getDimension();
    double result = 10.0 * n;
    for (int i = 0; i < n; ++i) {
      double xi = x.getValue(i); // i-th element of x.
      result += xi * xi - 10.0 * Math.cos(2 * Math.PI * xi);
    }
    return result;
  }
  ...
  private static void executeOneTrial(
      TSArexJgg ga, long maxEvals, TCTable log, String trialName, int trialNo) {
    ...
    putLogData(
        log, trialName, trialNo, logIndex, noOfEvals,
        best); // Saves the information of the last population in the log table.
    System.out.println(
        "TrialNo:"
            + trialNo
            + ", NoOfEvals:"
  ...
  public static void main(String[] args) throws IOException {
    ...
    String trialName =
        "ArexJggOffsetRastriginP14K5"; // The trial name, which is used for the log filename.
    String logFilename = "output/report04/" + trialName + ".csv"; // The log filename.

    ICRandom random = new TCJava48BitLcg(); // Random generator.
    TSArexJgg ga =
        new TSArexJgg(minimization, dimension, populationSize, noOfKids, random); // AREX/JGG
