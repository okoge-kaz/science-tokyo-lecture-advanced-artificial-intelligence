...
public class TCObjectFactory {
  ...
  public <T> T[] createArray(String key, Class<?> componentType, String defaultValue)
      throws Exception {
    String absKey = fProperties.getAbsoluteKey(key);
    String val = fProperties.getProperty(absKey);
    if (val == null) {
...
class TCConstructor {
  ...
  /**
   * プロパティファイルに指定されたキーがすべて見つかり，かつ， プロパティファイルに指定されなかったキーのすべてにデフォルト値が設定されている場合のみtrue そうでなければfalseを返す．
   *
   * @param propertyKeys
   * @return パラメータがマッチしたか
   * @since 2 hmkz
