...
public class TCMatrix implements Serializable, Cloneable {
  ...
  /** Constructor. Creates a matrix with size (0,0). */
  public TCMatrix() {
    this(0, 0);
  }

  /**
   * Constructor. Creates a matrix with specified size. All elements are initialized with zero.
   *
   * @param m
   * @param n
   */
  public TCMatrix(@ACParam(key = "RowDimension") int m, @ACParam(key = "ColumnDimension") int n) {
    fM = m;
    fN = n;
  ...
  /**
   * コンストラクタ． m次元縦ベクトル（m×1行列）として初期化する．
   *
   * @param m 縦ベクトルの次元数
   */
  public TCMatrix(@ACParam(key = "RowDimension") int m) {
    this(m, 1);
  }
  ...
  /**
   * Constructor. Creates a matrix with specified elements with deep-copying.
   *
   * @param elements
   */
  ...
  /**
   * Constructor. Creates a matrix by deep-copying the specified matrix.
   *
   * @param src
   */
  ...
  /**
   * 部分行列をコピーする． 行列 src の src_i0 から src_i1 行，および src_j0 から src_j1 列に対応する部分行列を， 自身の dst_i0 行， dst_j0
   * 列以降の部分にコピーする． もしコピー対象の部分行列が自身の行列に入りきらない場合，自身のサイズを拡大し， 拡大され，かつ部分行列がコピーされない要素には0が格納される．
   *
   * @param src TCMatrix
   ...
   * @return
   */
  public TCMatrix copySubmatrixFrom(
      TCMatrix src, int src_i0, int src_i1, int src_j0, int src_j1, int dst_i0, int dst_j0) {
  ...
  /**
   * Changes sizes of a matrix. All elements are reinitialized as zero.
   *
  ...
  /**
   * 値をインデックスで指定された要素へ設定する。 ここで、m×n行列Xにおいて、X(i, j)に値を設定したい場合、インデックスはi*n+jとなる。
   *
   * @param idx インデックス
   * @author isao
   */
  ...
  /**
   * Adds the specified matrix to this matrix. Updates this object. A = A + B.
   *
   * @param b
   * @return this matrix
   */
  ...
  /**
   * Adds the specified scalar value to each element of this matrix. Updates this object.
  ...
  /**
   * Subtructs the specified matrix from this matrix. Updates this object. A = A - B.
   *
   * @param b
   * @return this matrix
   */
  ...
  /**
   * Subtracts the specified saclar value from each element of this matrix. Updates this object.
   * A<sub>ij</sub> = A<sub>ij</sub> - d
   *
   * @param d
   * @return this matrix
  ...
  /**
   * Scalar product. Updates this object. A = r * A.
   *
   * @param r
   * @return this
  ...
  /**
   * Times. Updates this object. C = A * B. Notice that C.times(C, B) or C.times(A, C) isn't allowed
   * to use. In those case, you have to use times(TCMatrix b) or timesLeft(TCMatrix b) respectively.
   *
   * @param src1 行列A
   * @param src2 行列B
   * @return this 行列C
  ...
  /**
   * Times. Updates this object. A = A * B.
   *
   * @param b
   * @return this
   */
  ...
  /**
   * Returns a matrix represents this matrix times the specified scaler value. Updates this object.
   * A = d * A
   *
   * @param d
   * @return this matrix
  ...
  /**
   * Times from left. Updates this object. A = B * A.
   *
   * @param b
   * @return this matrix
   */
  ...
  /**
   * Times each element. The elements <i>a<sub>i,j</sub></i> = <i>a<sub>i,j</sub></i> *
   * <i>b<sub>i,j</sub></i>. Updates this object.
   *
   * @param b
   * @return this matrix
   */
  ...
  /**
   * Div each element. The elements <i>a<sub>i,j</sub></i> = <i>a<sub>i,j</sub></i> *
   * <i>b<sub>i,j</sub></i>. Updates this object.
   *
   * @param b
   * @return this matrix
  ...
  /**
   * Returns transpose. Updates this object.
   *
   * @return this matrix
   */
  ...
  public TCMatrix solve(TCMatrix B) {
    return (fM == fN
        ? (new TCLUDecomposition(this)).solve(B)
        : (new TCQRDecomposition(this)).solve(B));
  }

  /**
   * Returns an inverse of this matrix. Updates this object.
   *
   * @return this matrix
   */
  ...
  /**
   * L2ノルムを返す． このメソッドは列ベクトルに対してのみ適用可能．
   *
   * @return L2ノルム
   */
  ...
  /**
   * 単位ベクトルに強制する． このメソッドは列ベクトルに対してのみ適用可能．
   *
   * @return 単位ベクトル
   */
  ...
  public double innerProduct(TCMatrix v) {
    if (getColumnDimension() != 1
        ...
        || getRowDimension() != v.getRowDimension()) {
      throw new IllegalArgumentException("Dimensions are incorrect.");
    }
    double result = 0.0;
    for (int i = 0; i < getDimension(); ++i) {
  ...
  /**
   * Initializes this matrix as the identity matrix. The elements <i>x<sub>i,i</sub></i> = 1.0,
   * where <i>i</i> = min {<i>m, n</i>}, and the other elements are 0.
   *
   * @return this matrix
   */
  public TCMatrix eye() {
  ...
  /**
   * matrix exponential by using eigenvalue decomposition Note that this way is ``Not'' like matlab
   * that uses Pade approximant.
   *
  ...
  /**
   * m-by-n行列の最大値とそれに対応するインデックスを発見する． 最大値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す．
   *
   * @param indexes 最大値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意．
   * @return max 最大.
   * @author fnob
   */
  ...
  /**
   * m-by-n行列の行方向の最大値とそれに対応するインデックスを発見する． 最大値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す．
   *
   * @param indexes 最大値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意．
  ...
  /**
   * m-by-n行列の列方向の最大値とそれに対応するインデックスを発見する. 最大値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す.
   *
   * @param indexes 最大値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意.
  ...
  /**
   * m-by-n行列の最小値とそれに対応するインデックスを発見する． 最小値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す．
   *
   * @param indexes 最小値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意．
   * @return min 最小値.
   * @author fnob
   */
  ...
  /**
   * m-by-n行列の行方向の最小値とそれに対応するインデックスを発見する． 最小値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す．
   *
   * @param indexes 最小値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意．
  ...
  /**
   * m-by-n行列の列方向の最小値とそれに対応するインデックスを発見する． 最小値に対応するインデックスが複数ある場合，それらのインデックスを全て取り出す．
   *
   * @param indexes 最小値の『インデックス』を格納するリスト. ただし，m-by-n行列における(i,j)要素の『インデックス』は，i*n + jで表現されることに注意．
  ...
  /**
   * 最後の行の後に行列を追加する． 内部的には，毎回，メモリの確保と要素のコピーが起こるので効率はあまり良くない．
  ...
  /**
   * 最後の列の後に行列を追加する． 内部的には，毎回，メモリの確保と要素のコピーが起こるので効率はあまり良くない．
  ...
  public static void main(String[] args) {
    ...
    */
    ...
    //		mat2.eye();
    //		mat2.copySubmatrixFrom(mat2, 0, mat2.getRowDimension()-1, 0, mat2.getColumnDimension()-1, 0,
    // mat2.getColumnDimension());
