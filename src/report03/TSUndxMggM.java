...
/**
 * This program executes UNDX+MGG three trials, changing a random seed.
 ...
 * - Log filename: UndxMggKTabletP14K5.csv
 * 
 * @author isao
 *
 */
...
public class TSUndxMggM {
	...
	private static void initializePopulation(TCSolutionSet<TSRealSolution> population, double min, double max, ICRandom random) {
		for (TSRealSolution s: population) {
			s.getVector().rand(random).times(max - min).add(min); //å¬ëÃÇÃç¿ïWÇîÕàÕ[min, max]^nÇÃóêêîÇ≈èâä˙âªÅD
		}
	}
	...
	private static void executeOneTrial(TSUndxMgg ga, long maxEvals, TCTable log, String trialName, int trialNo) {
		...
		putLogData(log, trialName, trialNo, logIndex, noOfEvals, best); //Save the information of the last population in the log table.	
	...
	public static void main(String[] args) throws IOException {
		...
		String trialName = "UndxMggKTabletP14K5"; //The trial name, which is used for the log filename.
		String logFilename = trialName + ".csv"; //The log filename.
		
		ICRandom random = new TCJava48BitLcg(); //Random generator.
		TSUndxMgg ga = new TSUndxMgg(minimization, dimension, populationSize, noOfKids, random); //REX/JGG
		...
		log.writeTo(logFilename); //Output the log table containing the results of the three trials to the log file.
	}
