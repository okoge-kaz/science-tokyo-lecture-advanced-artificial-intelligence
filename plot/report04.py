...
# 手法のリスト
methods = {"REX/JGG": "RexJgg", "AREX/JGG": "ArexJgg"}
...
# 色とマーカーの設定
colors = {"REX/JGG": "blue", "AREX/JGG": "red"}
...
for method_name, method_code in methods.items():
    filepath = f"output/report04/{method_code}OffsetKTablet{P}{K}.csv"
...
plt.title(f"Comparison of REX/JGG vs AREX/JGG ({P}, {K})", fontsize=14)
plt.legend(bbox_to_anchor=(1.05, 1), loc="upper left")
plt.grid(True, which="both", ls="-", alpha=0.3)
plt.tight_layout()

# グラフの保存と表示
plt.savefig(f"figures/report04.png", dpi=300, bbox_inches="tight")
plt.show()
